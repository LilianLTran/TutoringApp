          ...
          {tutors.map((t) => (
            <tr key={t.id} className="transition-colors hover:bg-gray-50">
              <td className="px-6 py-4 font-medium text-gray-900">
                <Link href={`/dashboard/manager/tutors/${t.id}`} className="hover:text-black hover:underline">
                  {t.name}
                </Link>
              </td>
              ...
              <td className="px-6 py-4 text-center text-gray-700">{t.coursesCount}</td>
              <td className="px-6 py-4 text-center text-gray-700">{t.availabilityCount}</td>
              <td className="px-6 py-4 text-center text-gray-700">{t.sessionsCount}</td>

              <td className="px-6 py-4 text-right">
                <button
                  onClick={() => onDeleteClick(t)}
                  className="rounded-lg border px-3 py-1.5 text-xs font-medium text-red-700 hover:bg-red-50"
                >
                  Delete
                </button>
              </td>
